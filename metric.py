...
def get_metric(rgbs, targets, lpips=None, device=torch.device('cuda')):
    ...
    lpips_vgg(torch.from_numpy(targets).permute(2, 0, 1).unsqueeze(0).type(torch.FloatTensor).to(device), 
            torch.from_numpy(rgbs).permute(2, 0, 1).unsqueeze(0).type(torch.FloatTensor).to(device))

    lpips_score = torch.mean(lpips_vgg(torch.from_numpy(targets).permute(2, 0, 1).unsqueeze(0).type(torch.FloatTensor).to(device), 
                                        torch.from_numpy(rgbs).permute(2, 0, 1).unsqueeze(0).type(torch.FloatTensor).to(device)), dim=0).item()
