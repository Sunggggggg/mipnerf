...
def get_metric(rgbs, targets, lpips=None, device=torch.device('cuda')):
    ...
    lpips_score = torch.mean([lpips_vgg(torch.from_numpy(targets[i]).permute(2, 0, 1).unsqueeze(0).to(device), 
                            torch.from_numpy(rgbs[i]).permute(2, 0, 1).unsqueeze(0).to(device)) for i in range(test_view)]).item()
