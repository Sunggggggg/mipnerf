...
class NeRF(nn.Module):
    ...
    def forward(self, ray_batch):
        ...
        for l in range(self.num_levels):
            # sample
            if l == 0:  # coarse grain sample
                t_vals = torch.linspace(0., 1., steps=self.num_samples+1, device=ray_batch.device)
                t_vals = near * (1. - t_vals) + far * t_vals
                if self.randomized : 
                    mids = 0.5 * (t_vals[..., 1:] + t_vals[..., :-1])
                    upper = torch.cat([mids, t_vals[..., -1:]], -1)
                    lower = torch.cat([t_vals[..., :1], mids], -1)
                    t_rand = torch.rand(N_rays, self.num_samples + 1, device=ray_batch.device)
                    t_vals = lower + (upper - lower) * t_rand
                else :
                    t_vals = torch.broadcast_to(t_vals, [N_rays, self.num_samples + 1])
            ...
            # do integrated positional encoding of samples
        #     samples_enc = self.positional_encoding(mean)
        #     samples_enc = samples_enc.reshape([-1, samples_enc.shape[-1]])  # [N_rays*N_samples, 96]
